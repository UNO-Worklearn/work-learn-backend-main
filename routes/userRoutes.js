...
router.post("/activity/login", async (req, res) => {
  ...
  try {
    ...
    let log = user.activityLogs.find(
      (l) => l.date === date
    );
...
router.post("/activity/quiz", async (req, res) => {
  ...
  if (!user_id || !type) {
    return res.status(400).json({
      message: "Missing user_id or quiz type",
    });
  }
  ...
  try {
    ...
    let quiz = user.quizHistory.find(
      (q) => q.type === type
    );
    ...
    let dailyQuiz = log.quizzes.find(
      (q) => q.type === type
    );
    ...
    if (
      score >= 80 &&
      !dailyQuiz.reached80Percent
    ) {
      dailyQuiz.reached80Percent = true;
      dailyQuiz.attemptsToReach80 =
        dailyQuiz.attempts;
    }
